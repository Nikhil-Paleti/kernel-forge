...
__global__ void _stencil(float *input, float *output, int M, int N, int K) {
    ...
    __shared__ float in_prev[INPUT_TILE][INPUT_TILE];
    __shared__ float in_curr[INPUT_TILE][INPUT_TILE];
    __shared__ float in_next[INPUT_TILE][INPUT_TILE];
    ...
    if (
        ...
        in_prev[threadIdx.y][threadIdx.x] = input[load_k*M*N + j*N + i];
    }else{
        in_prev[threadIdx.y][threadIdx.x] = 0.0;
    }
    ...
    if (
        ...
        in_curr[threadIdx.y][threadIdx.x] = input[load_k*M*N + j*N + i];
    }else{
        in_curr[threadIdx.y][threadIdx.x] = 0.0;
    }
    ...
    for(int c=0; c<OUTPUT_TILE; c++){
        ...
        if (
            ...
            in_next[threadIdx.y][threadIdx.x] = input[next_k*M*N + j*N + i];
        }else{
             in_next[threadIdx.y][threadIdx.x] = 0.0;
        }
        ...
        if(
            ...
            if(
                ...
                output[current_k * M * N + j * N + i] = c0*in_curr[threadIdx.y][threadIdx.x] 
                                             +  c1*in_curr[threadIdx.y][threadIdx.x-1]
                                             + c2*in_curr[threadIdx.y][threadIdx.x+1]
                                             + c3*in_curr[threadIdx.y-1][threadIdx.x]
                                             + c4*in_curr[threadIdx.y+1][threadIdx.x]
                                             + c5*in_prev[threadIdx.y][threadIdx.x]
                                             + c6*in_next[threadIdx.y][threadIdx.x];
            }
            
        }else if (i >= 0 && i < N &&
        j >= 0 && j < M &&
        current_k >= 0 && current_k < K ){
            output[current_k * M * N + j * N + i] = in_curr[threadIdx.y][threadIdx.x];
        }

        __syncthreads();

        in_prev[threadIdx.y][threadIdx.x] = in_curr[threadIdx.y][threadIdx.x];
        in_curr[threadIdx.y][threadIdx.x] = in_next[threadIdx.y][threadIdx.x];
