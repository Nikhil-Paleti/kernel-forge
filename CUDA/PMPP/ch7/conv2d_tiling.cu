...
__global__ void _conv2d(float* N, float *P ,int height, int width, int r){
    int x = blockIdx.x * OUTPUT_TILE_DIM + threadIdx.x - FILTER_RADIUS;
    int y = blockIdx.y * OUTPUT_TILE_DIM + threadIdx.y - FILTER_RADIUS;
    ...
    if ((tile_x >=0 && tile_x < OUTPUT_TILE_DIM) && (tile_y >=0 && tile_y < OUTPUT_TILE_DIM)){
        float acc = 0.0;
        for (int j=0; j<2*r+1; j++){
            for(int i=0; i<2*r+1; i++){
                acc += F[j*(2*r+1) + i] * N_s[threadIdx.y - FILTER_RADIUS+j][threadIdx.x - FILTER_RADIUS+i];
            }
        }
        if( (x >=0 && x < width) && (y >= 0 && y < height) ){
            P[y*width + x] = acc;
...
void conv2d(float *image, float *filter, float *output, int M, int N, int r){
    ...
    dim3 grid((N + OUTPUT_TILE_DIM - 1) / OUTPUT_TILE_DIM , (M+OUTPUT_TILE_DIM-1)/OUTPUT_TILE_DIM);
