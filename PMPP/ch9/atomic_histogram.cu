/**
 * @file atomic_histogram.cu
 * @brief GPU and CPU histogram comparison example using CUDA.
 *
 * computes histogram using naive atomic operations in cuda. 
 *
...
void histogram(char *data, unsigned int length, unsigned int *histo, int NUM_BINS){
...
int main() {
    const int NUM_BINS = 26 / BIN_SIZE;
    ...
    histogram(data, length, hist_GPU, NUM_BINS);
